...
public class Program6_DeepCopyOfLinkedListUsingRandomPointer {
	...
	static Node deepCopyOfList(Node head) {
		...
		// this while loop will map the node each other
		temp = head;
		while(temp!=null) {
			...
			Node random = hm.get(temp.random);
			nTemp.random = random;
	...
	public static void main(String[] args) {
	       ...
	       head.next.next.next.next.next = new Node(3);
	        ...
	        head.random = head.next.next; // 23 -> 27
	        head.next.random = head; // 14 -> 23
	        head.next.next.random = head.next.next.next.next; // 27 -> 8
	        head.next.next.next.random = head.next; // 9 -> 14
	        head.next.next.next.next.random = head; // 8 -> 23
